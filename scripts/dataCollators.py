...
class DataCollatorSpeechSeq2SeqWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        if (labels[:, 0] == self.processor.tokenizer.bos_token_id).all().cpu().item(): # if bos token is appended in previous tokenization step, cut bos token here as it's append later anyways
            labels = labels[:, 1:]
        batch["labels"] = labels
        logging.info(f'Step batch: input_features.shape={list(batch["input_features"].shape)} (batch_size, mel_filters, frames) labels.shape={list(batch["labels"].shape)} (batch_size, seq_len)')
        return batch